...
- [ ] **Decide: upgrade to Workers Paid (~US$5/mo) to raise the Worker's CPU-time limit, or shrink the indexer's per-tick workload to fit the Free plan's ~10ms CPU ceiling.** Root cause of the stalled 30-NSW-stores snapshot: some cron ticks are killed mid-run with `exceededCpu` before finishing a page's D1 upserts, so the nationwide cycle never progresses past NSW. Needs the user's decision (cost vs. slower indexing) — see HANDOFF.md.
