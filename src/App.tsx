...
export default function App() {
  ...
  return <><header className="site-header"><a href="#" className="brand" onClick={e => { e.preventDefault(); navigate('find'); }}><span className="brand-mark" aria-hidden="true">s<span>f</span></span>Stock Finder<span className="country">AU</span></a><nav aria-label="Main navigation">{[['find', 'Find stock'], ['stores', 'Stores'], ['products', 'Products']].map(([id, title]) => <button key={id} className={page === id && !selectedStore ? 'active' : ''} aria-current={page === id && !selectedStore ? 'page' : undefined} onClick={() => navigate(id)}>{title}</button>)}</nav><span className="header-note">A better way to find it.</span><LastUpdatedBadge status={syncStatus}/><button className="theme-toggle" type="button" onClick={toggleTheme} aria-label={effectiveDark ? 'Switch to light theme' : 'Switch to dark theme'}>{effectiveDark ? '☀' : '☾'}</button></header><main id="main"><Suspense fallback={<p className="empty" role="status">Loading…</p>}>
    {selectedStore ? <Inventory key={selectedStore.code} store={selectedStore} products={catalogue.variants} onBack={() => setSelectedStore(undefined)} onPage={remember}/> : page === 'products' ? <Products catalogue={catalogue} location={location} onCheck={next => { const merged = { ...filters, ...next }; setFilters(merged); navigate('find'); check(merged); }}/> : page === 'stores' ? <Stores stores={directory.map(s => ({ ...s, distanceMetres: distance(location, s) }))} favourites={favourites} toggleFavourite={toggleFavourite} onOpen={openStore}/> : <>
      ...
      {filteredStores.length > 0 && <StoreMap stores={filteredStores} stock={stock} center={location} onSelect={openStore}/>}
